...
export default function MoleculesMenuList() {
  const menuInfo = [
    {
    ...
    },
    {
      ...
      items: [
        ...
        { itemName: "ԱՄԲՈՂՋԱԿԱՆ ՇԻՇԸ ԳՆԵԼՈՒ ԴԵՊՔՈՒՄ ԿՍՏԱՆԱՔ", price: "20020% ԶԵՂՉ" }
      ]
    },
    {
      mainTitle: "ԳԱՐԵՋՈՒՐ",
      ...
      {menuInfo.map((category, catIndex) => (
        <div key={catIndex} className="menu_list_content w-[80%]">
          ...
          {category.items.map((item, itemIndex) => (
            <AtomLi
              key={itemIndex}
              className="montserrat-b w-full flex text-[#5a421f] py-[10px] text-[20px] min-[620px]:justify-between items-center max-[620px]:flex-col justify-center border-t-1 border-[#866a3f] last:border-b-1 border-[#866a3f]"
            >
              {item.itemName} <span className="montserrat-b text-[#5a421f]">{item.price}</span>
            </AtomLi>
          ))}
        </div>
      ))}
